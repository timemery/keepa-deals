...
def get_field(data, deal_data, product_90, header, field):
    ...
    if header.startswith("Amazon - "):
        ...
        if period_key and stats_source:
            value = stats_source.get(period_key, [-1] * 30)[4] if len(stats_source.get(period_key, [-1] * 30)) > 4 else -1
    ...
    if header in ["Buy Box - Lowest", "Buy Box - Highest"]:
        ...
        price = stats[10] if isinstance(stats, list) and len(stats) > 10 and stats[10] > 0 else -1
        return f"${price / 100:.2f}" if price > 0 else '-'
    ...
    if header in ["Used, like new - 30 days avg.", "Used, like new - 60 days avg.", "Used, like new - 90 days avg.", 
        ...
        avg = stats_365.get(period, [-1] * 30)[12]
    ...
    # ... (keep existing Deal found, last update, etc., unchanged)
...
# Chunk 8 starts
# Dev Log: Fixed TypeError in fetch_product, removed product_code/history, used get_field (2025-04-27).
import sys
import keepa
...
def fetch_product(asin, days, api):
    ...
    return api.query(asin, stats=days)[0]
