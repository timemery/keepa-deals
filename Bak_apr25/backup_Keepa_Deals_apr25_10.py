...
def fetch_deals(page):
    ...
    url = f"https://api.keepa.com/deal?key={api_key}&selection={urllib.parse.quote(json.dumps(deal_query))}"
...
def price_now(data):
    ...
    valid_prices = [offer['price'] for offer in offers if offer.get('price', -1) > 0 and offer.get('condition') in [2, 3, 4, 5]]
