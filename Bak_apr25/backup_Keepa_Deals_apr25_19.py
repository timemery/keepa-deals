...
def get_stat_value(stats, key, index, divisor=100):
...
def buy_box_used_current(product):
    stats = product.get('stats', {})
    return {'Buy Box Used - Current': get_stat_value(stats, 'current', 3)}

def write_csv(rows, diagnostic=False):
    with open('Keepa_Deals_Export.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Buy Box Used - Current'])
        if diagnostic:
            writer.writerow(['No deals fetched'])
        else:
            writer.writerows([[row['Buy Box Used - Current']] for row in rows])
