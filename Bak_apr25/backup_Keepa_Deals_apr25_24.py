...
def fetch_product(asin, days, api):
    ...
    try:
        ...
        logging.debug(f"Raw stats for ASIN {asin}: current={current}")
        if not stats or len(current) < 11:
            logging.error(f"Incomplete stats for ASIN {asin}: {stats}")
            return {'stats': {'current': [-1] * 30}, 'asin': asin}
        if current[10] <= 0:
            logging.warning(f"No Used price for ASIN {asin}: current[10]={current[10]}, trying current[3]={current[3]}")
        return product
    except Exception as e:
...
def buy_box_used_current(product):
    stats = product.get('stats', {})
    current = stats.get('current', [-1] * 30)
    value = current[10] if len(current) > 10 and current[10] > 0 else current[3] if len(current) > 3 and current[3] > 0 else -1
    return {'Buy Box Used - Current': get_stat_value(stats, 'current', 10 if value == current[10] else 3, 1000)}
    ...
    with open('Keepa_Deals_Export.csv', 'w', newline='', encoding='utf-8') as f:
        ...
        if diagnostic:
            ...
            for row, deal in zip(rows, deals[:len(rows)]):
                writer.writerow([deal['asin'] if header == 'ASIN' else deal['title'] if header == 'Title' else row.get(header, '-') for header in HEADERS])
...
def main():
    try:
        ...
        for deal in deals[:3]:
            ...
            rows.append(buy_box_used_current(product))
