...
def fetch_product(asin, days, api):
    ...
    try:
        product = api.query(asin, stats=days, offers=20, rating=True, update=0, to_csv=False, stock=True)[0]
        if not product:
...
def price_now(data):
    ...
    used_indices = [7, 11, 13, 15, 17]  # Buy Box Used, Used, Like New, Very Good, Good
    valid_prices = [stats[i] for i in used_indices if i < len(stats) and stats[i] > 0]
    logging.debug(f"Price Now for ASIN {data.get('asin', '-')}: valid_prices={valid_prices}")
    if not valid_prices:
        return '-'
    value = min(valid_prices)
    if value > 100000:  # Reject outliers
        return '-'
    return f"${value / 100:.2f}"

def validate_header(asin, header, value, api):
    product = api.query(asin, stats=365, offers=20, rating=True, update=0, to_csv=False, stock=True)[0]
    stats = product.get('stats', {}).get('current', [-1] * 30)
    valid_prices = [stats[i] for i in [7, 11, 13, 15, 17] if i < len(stats) and stats[i] > 0]
