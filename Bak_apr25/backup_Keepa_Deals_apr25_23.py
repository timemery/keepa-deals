...
def fetch_product(asin, days, api):
    ...
    try:
        ...
        logging.debug(f"Raw stats for ASIN {asin}: current={stats.get('current', [])}")
        if not stats or len(stats.get('current', [])) < 11:
            logging.error(f"Incomplete stats for ASIN {asin}: {stats}")
            return {'stats': {'current': [-1] * 30}, 'asin': asin}
        if stats.get('current', [-1] * 30)[10] <= 0:
            logging.warning(f"No Used price for ASIN {asin}: current={stats.get('current', [])}")
        return product
    except Exception as e:
...
def write_csv(rows, deals, diagnostic=False):
    with open('Keepa_Deals_Export.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        headers = ['ASIN', 'Title'] + HEADERS
        writer.writerow(headers)
        if diagnostic:
            writer.writerow(['No deals fetched', '-'] + ['-'] * len(HEADERS))
        else:
            for row, deal in zip(rows, deals[:len(rows)]):
                writer.writerow([deal['asin'], deal['title']] + [row.get(header, '-') for header in HEADERS])
