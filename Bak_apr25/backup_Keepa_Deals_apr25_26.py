...
def fetch_product(asin, days, api):
    ...
    try:
        ...
        if not stats or len(current) < 12:
            logging.error(f"Incomplete stats for ASIN {asin}: {stats}")
            return {'stats': {'current': [-1] * 30}, 'asin': asin}
        if current[11] <= 0:
            logging.warning(f"No Used price for ASIN {asin}: current[11]={current[11]}")
        return product
    except Exception as e:
...
def sales_rank_current(product):
    stats = product.get('stats', {})
    value = stats.get('current', [-1] * 30)[2]
    return {'Sales Rank - Current': str(value) if value > 0 else '-'}

def buy_box_used_current(product):
    stats = product.get('stats', {})
    return {'Buy Box Used - Current': get_stat_value(stats, 'current', 11)}
