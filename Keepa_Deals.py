...
def check_tokens():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.debug(f"Token check: tokensLeft={tokens_left}")
            return tokens_left > 0
        logging.error(f"Token check failed: {response.status_code}, {response.text}")
        return False
    except Exception as e:
        logging.error(f"Token check exception: {str(e)}")
        return False
# Chunk 1 ends
...
def fetch_product(asin, days=365, offers=10, rating=1, history=1):
    ...
    if not check_tokens():
        logging.error(f"Insufficient tokens for ASIN {asin}")
        print(f"Insufficient tokens for ASIN {asin}")
        return {'stats': {'current': [-1] * 30}, 'asin': asin}
...
# New, 3rd Party FBM - Stock is "coincidentally" correct... not sure why... 
def new_3rd_party_fbm(product):
    stats = product.get('stats', {})
    asin = product.get('asin', 'unknown')
    offers = product.get('offers', [])
    stock = sum(1 for o in offers if o.get('condition', -1) == 1 and not o.get('isFBA', False) and o.get('stock', -1) > 0)  # New, non-FBA
    result = {
        ...
        'New, 3rd Party FBM - Stock': str(stock)  # Note: Matches live Keepa (all 0), possibly correct
    }
