...
def fetch_deals(page):
    ...
    deal_query = {
        ...
        "sortType": 4,
        "dateRange": "3",
        "warehouseConditions": [2, 3, 4, 5]  # Used conditions
    }
...
def fetch_product(asin, days=365, offers=0, rating=1, history=1):
    ...
    try:
        ...
        logging.debug(f"Full stats for ASIN {asin}: {stats}")
        logging.debug(f"Stats keys: {list(stats.keys())}, current={current}, avg30={stats.get('avg30', [-1] * 30)}, outOfStock={stats.get('outOfStock', [-1] * 30)}")
