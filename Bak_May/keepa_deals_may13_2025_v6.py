...
def fetch_product(asin, api_key, days=365, offers=20, rating=1, history=1):
    ...
    try:
        ...
        logging.debug(f"Response status: {response.status_code}, response_text={response.text[:500]}")
        if response.status_code != 200:
            logging.error(f"Request failed: {response.status_code}, {response.text[:200]}")
