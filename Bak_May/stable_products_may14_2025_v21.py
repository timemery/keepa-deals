...
def publication_date(product):
    ...
    asin = product.get('asin', 'unknown')
    logging.debug(f"Publication Date - raw value={value}, type={type(value)}, product_keys={list(product.keys())} for ASIN {asin}")
    ...
    try:
        ...
        logging.error(f"publication_date failed for ASIN {asin}: {str(e)}")
        return {'Publication Date': '-'}
