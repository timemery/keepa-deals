...
from stable_products import (
    ...
    get_asin_from_product, get_title_from_product, package_quantity_from_product,
...
def fetch_product(asin, days=365, offers=20, rating=1, history=1):
    ...
    try:
        ...
        logging.debug(f"Response status: {response.status_code}, response_text={response.text[:200]}")
        if response.status_code != 200:
            logging.error(f"Request failed: {response.status_code}, {response.text[:100]}")
    ...
    try:
        ...
        for deal in deals:
            ...
            try:
                ...
                row.update(get_asin_from_product(product))
                row.update(get_title_from_product(product))
                row.update(package_quantity_from_product(product))
