...
def get_asin_local(product):
    asin = product.get('asin', '-')
    result = {'ASIN': f'="{asin}"' if asin != '-' else '-'}
    logging.debug(f"get_asin_local result for ASIN {asin}: {result}")
    return result

def get_title_local(product):
    title = product.get('title', '-')
    result = {'Title': title}
    logging.debug(f"get_title_local result for ASIN {product.get('asin', 'unknown')}: {title[:50]}")
    return result

def get_package_quantity_local(product):
    quantity = product.get('packageQuantity', -1)
    result = {'Package - Quantity': str(quantity) if quantity != -1 else '-'}
    logging.debug(f"get_package_quantity_local result for ASIN {product.get('asin', 'unknown')}: {result}")
    return result
