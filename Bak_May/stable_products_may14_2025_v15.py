...
def publication_date(product):
    ...
    logging.debug(f"Publication Date - raw ts={ts}")
    if ts <= 0:
        logging.error(f"No valid publicationDate for ASIN {product.get('asin', 'unknown')}")
        return {'Publication Date': '-'}
    try:
        dt = datetime.fromtimestamp(ts / 1000)
        formatted = TORONTO_TZ.localize(dt).strftime('%Y-%m-%d')
        logging.debug(f"Publication Date result for ASIN {product.get('asin', 'unknown')}: {formatted}")
        return {'Publication Date': formatted}
    except Exception as e:
        logging.error(f"publication_date failed: {str(e)}")
        return {'Publication Date': '-'}
