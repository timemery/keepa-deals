...
def buy_box_current(product):
    ...
    value = current[0]
    logging.debug(f"Buy Box - Current - raw value={value}, current array={current} for ASIN {asin}")
    if value <= 0:
        logging.info(f"No valid Buy Box - Current (value={value}), retrying for ASIN {asin}")
        try:
            retry_product = fetch_product_for_retry(asin)
            retry_stats = retry_product.get('stats', {})
            retry_current = retry_stats.get('current', [-1] * 20)
            value = retry_current[0]
            logging.debug(f"Buy Box - Current retry - raw value={value}, current array={retry_current} for ASIN {asin}")
        except Exception as e:
            logging.error(f"Retry failed for ASIN {asin}: {str(e)}")
            return {'Buy Box - Current': '-'}
    if value <= 0:
        logging.info(f"No valid Buy Box - Current after retry (value={value}) for ASIN {asin}")
        return {'Buy Box - Current': '-'}
    try:
...
def amazon_current(product):
    ...
    if value <= 0:
        logging.info(f"No valid Amazon - Current (value={value}), retrying for ASIN {asin}")
        try:
            retry_product = fetch_product_for_retry(asin)
            retry_stats = retry_product.get('stats', {})
            retry_current = retry_stats.get('current', [-1] * 20)
            value = retry_current[10]
            logging.debug(f"Amazon - Current retry - raw value={value}, current array={retry_current} for ASIN {asin}")
        except Exception as e:
            logging.error(f"Retry failed for ASIN {asin}: {str(e)}")
            return {'Amazon - Current': '-'}
    if value <= 0:
        logging.info(f"No valid Amazon - Current after retry (value={value}) for ASIN {asin}")
        return {'Amazon - Current': '-'}
    try:
        formatted = f"${value / 100:.2f}"
