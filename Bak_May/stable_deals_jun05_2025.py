...
# Deal Found starts
def deal_found(deal):
    logging.debug(f"deal_found input: {deal}")  # Log full deal object
    deal_date = deal.get('dealDate', deal.get('startDate', deal.get('timestamp', -1)))
    if deal_date == -1:
        logging.error(f"No dealDate/startDate/timestamp for deal: {deal}")
        return {'Deal found': '-'}
    try:
