...
from stable import get_stat_value, buy_box_used_current, sales_rank_current
    ...
    with open('Keepa_Deals_Export.csv', 'w', newline='', encoding='utf-8') as f:
        ...
        if diagnostic:
            ...
            for row, deal in zip(rows, deals[:len(rows)]):
                ...
                writer.writerow([f'="{deal["asin"]}"' if header == 'ASIN' else deal['title'] if header == 'Title' else row_data.get(header, '-') for header in HEADERS])
# Chunk 4 ends
