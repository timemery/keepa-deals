...
def buy_box_used_current(product):
    stats = product.get('stats', {})
    # Try current[18] (Buy Box Used), then [3] (Marketplace Used), then [1] (Marketplace New)
    result = {'Buy Box Used - Current': get_stat_value(stats, 'current', 18, divisor=100, is_price=True)}
    if result['Buy Box Used - Current'] == '-':
        result['Buy Box Used - Current'] = get_stat_value(stats, 'current', 3, divisor=100, is_price=True)
        logging.debug(f"Fell back to stats.current[3] for Buy Box Used - Current")
    if result['Buy Box Used - Current'] == '-':
...
def sales_rank_current(product):
    stats = product.get('stats', {})
    result = {'Sales Rank - Current': get_stat_value(stats, 'current', 2, divisor=1, is_price=False)}
