...
def fetch_product(asin, days, api):
    ...
    try:
        ...
        if not stats or len(current) < 11:
            ...
            return {'stats': {'current': [-1] * 30}, 'asin': asin}
        if current[3] <= 0:
            logging.warning(f"No Used price for ASIN {asin}: current[3]={current[3]}")
        if current[2] <= 0:
            logging.warning(f"No Sales Rank for ASIN {asin}: current[2]={current[2]}")
        return product
    except Exception as e:
...
def used_current(product):
    stats = product.get('stats', {})
    return {'Used - Current': get_stat_value(stats, 'current', 3, divisor=100, is_price=True)}

def sales_rank_current(product):
    stats = product.get('stats', {})
    return {'Sales Rank - Current': get_stat_value(stats, 'current', 2, divisor=1, is_price=False)}
