...
def fetch_product(asin, days, api):
    ...
    try:
        ...
        if not stats or len(current) < 11:
            ...
            return {'stats': {'current': [-1] * 30}, 'asin': asin}
        if current[1] <= 0:
            logging.warning(f"No Buy Box price for ASIN {asin}: current[1]={current[1]}")
        return product
    except Exception as e:
...
def buy_box_used_current(product):
    stats = product.get('stats', {})
    return {'Buy Box Used 🚚 - Current': get_stat_value(stats, 'current', 1, divisor=100, is_price=True)}
